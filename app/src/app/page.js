...
function MechCard({ title, children, gold }) {
  return (
    <div style={{padding:20,border:`1px solid ${gold?"rgba(255,215,0,0.2)":"rgba(22,82,240,0.15)"}`,borderRadius:8,background:gold?"rgba(255,215,0,0.03)":"rgba(22,82,240,0.03)",display:"flex",flexDirection:"column",gap:10}}>
...
function Divider() {
  return <div style={{height:1,background:"linear-gradient(90deg,transparent,rgba(22,82,240,0.2),transparent)"}}/>;
}
    ...
    <div style={{fontFamily:"'JetBrains Mono',monospace",background:"radial-gradient(ellipse at 30% 0%,#0D1A30 0%,#080E1C 45%,#060A14 100%)",minHeight:"100vh",color:"#c8d6e5",position:"relative"}}>
      ...
      <section style={{position:"relative",zIndex:5,padding:"56px 20px 40px",maxWidth:680,margin:"0 auto"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:10,marginBottom:20}}>
          <div style={{position:"relative",padding:12,border:"1px solid rgba(22,82,240,0.18)",borderRadius:10,background:"rgba(8,14,28,0.7)"}}>
            {[["tl",{top:0,left:0,borderLeft:"2px solid rgba(22,82,240,0.5)",borderTop:"2px solid rgba(22,82,240,0.5)"}],
              ["tr",{top:0,right:0,borderRight:"2px solid rgba(22,82,240,0.5)",borderTop:"2px solid rgba(22,82,240,0.5)"}],
              ["bl",{bottom:0,left:0,borderLeft:"2px solid rgba(22,82,240,0.5)",borderBottom:"2px solid rgba(22,82,240,0.5)"}],
              ["br",{bottom:0,right:0,borderRight:"2px solid rgba(22,82,240,0.5)",borderBottom:"2px solid rgba(22,82,240,0.5)"}]
        ...
        <div style={{width:"100%",border:"1px solid rgba(22,82,240,0.18)",borderRadius:10,background:"rgba(8,14,28,0.8)",overflow:"hidden"}}>
          {!codeSuccess ? (
            <>
              <div style={{padding:"12px 18px",borderBottom:"1px solid rgba(22,82,240,0.1)",background:"rgba(22,82,240,0.04)",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              ...
              <div style={{padding:"14px 18px",display:"flex",flexDirection:"column",gap:9}}>
                <div style={{position:"relative"}}>
                  <input type="text" value={code} placeholder="XXXX-XXXX" maxLength={9} autoComplete="off" spellCheck={false}
                    ...
                    style={{width:"100%",background:"rgba(0,0,0,0.5)",border:`1px solid ${codeError?"rgba(255,51,85,0.5)":"rgba(22,82,240,0.2)"}`,borderRadius:7,padding:"12px",fontFamily:"Orbitron,sans-serif",fontSize:20,fontWeight:700,color:"#e0e8f0",textAlign:"center",letterSpacing:6,outline:"none",display:"block",caretColor:"#3B7BF6"}}/>
                  {showCursor&&!code&&(
                ...
                {codeError&&<div style={{fontSize:10,color:"#ff3355",textAlign:"center"}}>{codeError}</div>}
                <button onClick={redeemCode} disabled={codeLoading} style={{width:"100%",fontFamily:"Orbitron,sans-serif",fontSize:11,fontWeight:700,padding:12,borderRadius:7,border:"none",background:"linear-gradient(135deg,#1652F0,#3B7BF6)",color:"#fff",cursor:"pointer",letterSpacing:1.5,opacity:codeLoading?0.7:1}}>
            ...
            <div style={{padding:"24px 18px",display:"flex",flexDirection:"column",alignItems:"center",gap:12,textAlign:"center"}}>
              ...
              <button onClick={()=>router.push("/play")} style={{width:"100%",fontFamily:"Orbitron,sans-serif",fontSize:11,fontWeight:700,padding:12,borderRadius:7,border:"none",background:"linear-gradient(135deg,#1652F0,#3B7BF6)",color:"#fff",cursor:"pointer",letterSpacing:1.5}}>ENTER THE GRID →</button>
            </div>
          )}
        </div>
      </section>
      ...
      <section id="how-section" style={{position:"relative",zIndex:5,padding:"60px 20px",maxWidth:960,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:40}}>
          <div style={{fontSize:10,letterSpacing:3,color:"#3B7BF6",fontWeight:700,marginBottom:8}}>HOW IT WORKS</div>
          <div style={{fontFamily:"Orbitron,sans-serif",fontSize:24,fontWeight:700,letterSpacing:1,color:"#e0e8f0"}}>Four Steps to Win</div>
        </div>
        <div className="steps-grid">
          {[
            ...
            <div key={n} style={{padding:20,border:"1px solid rgba(22,82,240,0.15)",borderRadius:8,background:"rgba(22,82,240,0.03)",display:"flex",flexDirection:"column",gap:10}}>
              ...
              <div style={{fontFamily:"Orbitron,sans-serif",fontSize:11,fontWeight:700,color:"#e0e8f0",letterSpacing:1}}>{t}</div>
      ...
      <section style={{position:"relative",zIndex:5,padding:"60px 20px",maxWidth:960,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:40}}>
          <div style={{fontSize:10,letterSpacing:3,color:"#3B7BF6",fontWeight:700,marginBottom:8}}>GAME MECHANICS</div>
          <div style={{fontFamily:"Orbitron,sans-serif",fontSize:24,fontWeight:700,letterSpacing:1,color:"#e0e8f0"}}>Know the Rules</div>
        </div>
        ...
        <div style={{border:"1px solid rgba(22,82,240,0.15)",borderRadius:10,background:"rgba(22,82,240,0.03)",padding:"32px 28px",display:"flex",flexDirection:"column",gap:32}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:4}}>
            {ZK_STEPS.map((label,i)=>{
              ...
              return (
                <div key={i}>
                  <div style={{display:"flex",alignItems:"center",gap:10,padding:"9px 12px",borderRadius:6,border:`1px solid ${active?"rgba(22,82,240,0.2)":done?"rgba(0,204,136,0.15)":"transparent"}`,background:active?"rgba(22,82,240,0.06)":done?"rgba(0,204,136,0.04)":"transparent",transition:"all 0.3s"}}>
                    <div style={{width:7,height:7,borderRadius:"50%",flexShrink:0,background:active?"#3B7BF6":done?"#00cc88":"rgba(22,82,240,0.2)",boxShadow:active?"0 0 6px #3B7BF6":"none",animation:active?"pulse 1.5s ease-in-out infinite":"none"}}/>
                    <span style={{fontSize:11,color:active?"#3B7BF6":done?"#00cc88":"#4a5a6e",fontWeight:active||done?600:400}}>{label}</span>
                  </div>
                  {i<ZK_STEPS.length-1&&<div style={{width:1,height:8,background:"rgba(22,82,240,0.15)",marginLeft:16}}/>}
                </div>
              );
            })}
          </div>
