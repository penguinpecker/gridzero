...
/// @title GridZero — 5x5 grid, USDC entry, winners paid automatically on resolve
/// @notice UUPS-upgradeable. Game mechanics are unchanged from the prior
///         non-upgradeable deployment; the only difference is that this sits
///         behind a proxy so logic can be fixed without stranding funds.
contract GridZero is
    Initializable,
    OwnableUpgradeable,
    UUPSUpgradeable
{
    ...
    // OpenZeppelin dropped ReentrancyGuardUpgradeable in contracts-upgradeable
    // 5.6, so the guard lives in this contract's own storage layout.
    ...
    // NOTE: not immutable — immutables live in code, which a proxy does not run.
    IERC20 public usdc;
    ...
    function initialize(
        ...
        require(_fulfiller != address(0), "fulfiller=0");
        ...
        entryFee = 1e6;              // 1 USDC
        roundDuration = 30;          // 30s
        protocolFeeBps = 500;        // 5%
        resolverReward = 0.1e6;      // 0.1 USDC
    ...
    // ══════════════════════════════════════════════════════════════
    // Resolver — auto-pays winners during resolve
    // ══════════════════════════════════════════════════════════════
    ...
    function resolveRound(bytes32 vrfOutput, uint256 roundId) external nonReentrant {
        ...
        if (isBonus) {
            ...
            uint256 available = balance > accumulatedFees ? balance - accumulatedFees : 0;
    ...
    function setFulfiller(address _v) external onlyOwner { require(_v != address(0), "zero"); fulfiller = _v; }
    function setFeeRecipient(address _v) external onlyOwner { require(_v != address(0), "zero"); feeRecipient = _v; }
    ...
    function setRoundDuration(uint256 _v) external onlyOwner { require(_v > 0, "zero"); roundDuration = _v; emit ConfigUpdated("roundDuration", _v); }
